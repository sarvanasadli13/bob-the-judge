...
class FunctionScore:
    ...
    @property
    def confidence_band(self) -> dict[str, float]:
        """
        Wilson confidence interval as a dict with low, high, and width.
        
        Returns:
            dict with keys:
                - low: Lower bound of 95% CI (percentage)
                - high: Upper bound of 95% CI (percentage)
                - width: Width of the interval (high - low)
        
        A narrower width indicates higher statistical confidence in the parity rate.
        """
...
def score_results(results: list[dict]) -> list[dict]:
    """
    Score parity test results by transaction type.
    
    Args:
        results: List of parity test result dicts
        
    Returns:
        List of scored function dicts, sorted by readiness_score descending
        
    Example:
        >>> results = [
        ...     {"transaction_type": "domestic", "parity": True, "divergences": [], 
        ...      "fee_diff": 0.0, "settled_diff": 0.0, "latency_improvement_pct": 15.0},
        ...     {"transaction_type": "domestic", "parity": False, "divergences": [{"severity": "HIGH"}],
        ...      "fee_diff": 2.5, "settled_diff": 0.0, "latency_improvement_pct": 10.0}
        ... ]
        >>> scores = score_results(results)
        >>> scores[0]["function"]
        'Domestic Wire Transfer'
        >>> scores[0]["parity_rate_pct"]
        50.0
        >>> scores[0]["confidence_band"]
        {'low': 9.4, 'high': 90.6, 'width': 81.2}
    """
    buckets: dict[str, FunctionScore] = defaultdict(lambda: FunctionScore(name=""))

    for r in results:
        tx_type = r["transaction_type"]
        func_name = TX_TYPE_TO_FUNCTION.get(tx_type, tx_type) or tx_type
...
def score_by_tenant(results: list[dict], tenant_id: str) -> list[dict]:
    """
    Score parity test results filtered by tenant_id.
    
    This function is useful for multi-entity banks where different tenants
    (e.g., retail vs. commercial banking divisions) need separate parity
    assessments before migrating to the modern platform.
    
    Args:
        results: List of parity test result dicts (must include 'tenant_id' field)
        tenant_id: The tenant identifier to filter by
        
    Returns:
        List of scored function dicts for the specified tenant,
    ...
    Example:
        ...
        ...      "settled_diff": 1.0, "latency_improvement_pct": 8.0}
        ... ]
        >>> retail_scores = score_by_tenant(results, "retail")
        >>> len(retail_scores)
        1
        >>> retail_scores[0]["function"]
        'Domestic Wire Transfer'
        >>> retail_scores[0]["parity_rate_pct"]
        100.0
        >>> retail_scores[0]["confidence_band"]["width"] < 20  # Tight confidence band
        True
    """
    filtered = [r for r in results if r.get("tenant_id") == tenant_id]
    return score_results(filtered)
